...
function isImageSupported() {
  var acceptHeader = arguments.length <= 0 || arguments[0] === undefined ? '' : arguments[0];
...
function canRequestWebp(acceptHeader, extension) {
  var webpExtensions = ['jpg', 'png', 'jpeg'];
...
function isFile(path) {
  return new Promise(function (resolve, reject) {
    ...
    (0, _fs.stat)(path, function (err, stats) {
      if (!err && stats.isFile()) {
        resolve();
      } else {
        reject();
...
function updateRequestUrl(pathname, dirname, extension, request, response, callback) {
  var updatedPath = pathname.replace(/\.[a-z]{3,4}$/i, '.' + extension);
  var filePath = (0, _path.join)(dirname, updatedPath);

  isFile(filePath).then(function () {
    request.url = request.url.replace(pathname, updatedPath);
    (0, _vary2.default)(response, 'Accept');
...
function rewriteImageRequestMiddleware(dirname) {
  return function rewriteImageRequest(request, response, next) {
    if (!isAcceptedMethod(request.method)) {
      ...
      var extension = (pathname.match(/\.([a-z]{3,4})$/i) || [])[1];
...
module.exports = {
  _private: {
    ...
    isAcceptedMethod: isAcceptedMethod,
    canRequestWebp: canRequestWebp,
    canRequestJxr: canRequestJxr,
    isFile: isFile,
    updateRequestUrl: updateRequestUrl
  },
  rewriteImageRequestMiddleware: rewriteImageRequestMiddleware
};
